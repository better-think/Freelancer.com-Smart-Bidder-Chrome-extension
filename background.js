...
{
    $.post(currentGateway + "/add_processed_switch_urls.php", {
...
{
    $.post(currentGateway + "/add_messages_statistics.php", {
               ...
               // console.log(response);
                 ...
                 if(response.status == "success")
                 {
                     console.log("SUCCESS! PRODUCT KEY SUCCESSFULLY VERIFIED!");             
    					   ...
    					   chrome.tabs.sendMessage(sender.tab.id, {action: "open_dialog_box", response :response }, function(response) {});  
        	});
			})
		});
...
function setProfitMessageProcessedByTheUser(message_id)
{
    ...
    $.post(currentGateway + "/messages_for_users.php", {
...
function processCheckingMessagesForUsers(sender)
{
    $.post(currentGateway + "/messages_for_users.php", {
      ...
      chrome.tabs.sendMessage(sender.tab.id, {action: "response_about_messages_from_database", response : response }, function(response) {}); 
    });
}
...
function processCheckingProfitMessagesForUsers(sender)
{
    $.post(currentGateway + "/messages_for_users.php", {
      ...
      chrome.tabs.sendMessage(sender.tab.id, {action: "response_about_profit_messages_from_database", response : response }, function(response) {}); 
    });
}
        ...
        function success(position) {
          ...
          callback(position);
            ...
            function error(error_message) {
              alert("We are sorry, but your location is Required for security purposes!Please, allow access to it via settings.");
              fetchUsersGeolocationOverall();
            } 
            );
    } else {
      alert("To buy this extension you need to download latest version of Chome that has geolocation api!");
    }
}
